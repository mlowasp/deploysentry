...
<style>
body{background:#05070d;color:#d8fff8;font-family:Inter,system-ui,monospace;margin:0;padding:32px} .wrap{max-width:1280px;margin:auto}
...
table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid #1e3d4a;padding:8px;text-align:left;vertical-align:top}.critical{color:#ff2e63}.high{color:#ff8a00}.medium{color:#ffd166}.low{color:#55ff99}.info{color:#39fff3} code{color:#55ff99} a{color:#39fff3}.pill{display:inline-block;border:1px solid #39fff3;border-radius:999px;padding:2px 8px;margin:2px;color:#39fff3}.evidence{color:#aee8ee;font-size:.92em}
</style></head><body><div class="wrap"><h1>DEPLOYSENTRY // Report</h1>
<div class="card"><h2>{{ r.target_domain }}</h2><p>Started: {{ r.started_at }}<br>Finished: {{ r.finished_at }}<br>Subdomains: {{ r.subdomains|length }}<br>Live services: {{ r.services|length }}<br>Technologies detected: {{ r.technologies|length }}<br>Shodan passive hosts: {{ r.shodan_hosts|length }}</p></div>
...
<div class="card"><h2>Technology Fingerprints</h2><p>CMS, hosted builders, e-commerce platforms, frameworks, and exposed admin surfaces detected from passive page signals plus a capped set of safe known-path probes.</p>{% if not r.technologies %}<p>No technology fingerprints detected.</p>{% else %}<table><tr><th>Technology</th><th>Version</th><th>Category</th><th>Asset</th><th>URL</th><th>Confidence</th><th>Evidence</th></tr>{% for t in r.technologies|sort(attribute='asset') %}<tr><td><span class="pill">{{t.name}}</span></td><td>{{t.version or "—"}}</td><td>{{t.category}}</td><td>{{t.asset}}</td><td><code>{{t.url}}</code></td><td>{{"%.0f"|format(t.confidence * 100)}}%</td><td class="evidence">{% for e in t.evidence[:6] %}{{e}}{{ '<br>' if not loop.last else '' }}{% endfor %}</td></tr>{% endfor %}</table>{% endif %}</div>
